...
(function(){
  ...
  var TOKK='kafi_gh_token', AR_REPO='khoakafi/khoakafi.github.io';
  if (!ls(TOKK)) return;
  ...
  async function deriveKey(tok,usage){ var base=await crypto.subtle.importKey('raw',new TextEncoder().encode(tok),'PBKDF2',false,['deriveKey']); return crypto.subtle.deriveKey({name:'PBKDF2',salt:new TextEncoder().encode('kafi-core-v1-salt'),iterations:100000,hash:'SHA-256'},base,{name:'AES-GCM',length:256},false,usage); }
  async function loadEngine(tok){ var r=await fetch('/assets/core.bin?cb='+Date.now(),{cache:'no-store'}); if(!r.ok) throw new Error('chua co core ('+r.status+')'); var raw=new Uint8Array(await r.arrayBuffer()); var key=await deriveKey(tok,['decrypt']); var pt=await crypto.subtle.decrypt({name:'AES-GCM',iv:raw.slice(0,12)},key,raw.slice(12)); (new Function(new TextDecoder().decode(pt)))(); if(!window.__GEN||!window.__GEN.run) throw new Error('core khong hop le'); }
  async function run(){
    ...
    try{
      ...
      var cur=await (await fetch('https://api.github.com/repos/'+AR_REPO+'/contents/signals_data.js',{headers:H2})).json();
      var body={message:'[AUTO] phat hanh tin hieu '+new Date().toISOString().slice(0,10),content:b64enc(sigJs)};
      if(cur&&cur.sha) body.sha=cur.sha;
      var pr=await fetch('https://api.github.com/repos/'+AR_REPO+'/contents/signals_data.js',{method:'PUT',headers:Object.assign({'Content-Type':'application/json'},H2),body:JSON.stringify(body)});
